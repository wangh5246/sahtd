...
import argparse, random, numpy as np, pandas as pd, json
...
# ================= 一些小工具 ================= #
# 在run_one_suite中修改RMSE计算：
def _compute_rmse_properly(df):
    """Remove warm-up period before computing RMSE"""
    WARMUP_ROUNDS = 5

    rmse_series = pd.to_numeric(df['rmse'], errors='coerce')

    if len(rmse_series) <= WARMUP_ROUNDS:
        return float('nan')

    # 只用warm-up之后的数据
    rmse_valid = rmse_series.iloc[WARMUP_ROUNDS:]

    return {
        'rmse_after_warmup': rmse_valid.mean(),
        'rmse_warmup_period': rmse_series.iloc[:WARMUP_ROUNDS].mean(),
        'convergence_rate': rmse_valid.iloc[0] - rmse_valid.iloc[-1]
    }
def infer_dataset_meta_from_reports(rep: pd.DataFrame):
    """
    ★ 新增：
    从 reports.csv 自动推断一些数据集级别的 meta 信息，用于自动设定 suite：
    - epsilon_data: 如果有 epsilon 列，就取众数/第一个
    - rho_data:     如果有 rho 列，就取众数/第一个
    - adv_frac:     如果有 label 列，统计 label=='adv' 的比例
    - adv_rate_col: 如果有 adv_rate 列，就取众数
    - n_workers:    reports.csv 里全局唯一 worker 数量
    """
    meta = {}

    if "epsilon" in rep.columns:
        eps = rep["epsilon"].dropna()
        if not eps.empty:
            meta["epsilon_data"] = float(eps.mode().iloc[0])
    if "rho" in rep.columns:
        rr = rep["rho"].dropna()
        if not rr.empty:
            meta["rho_data_col"] = float(rr.mode().iloc[0])
    if "adv_rate" in rep.columns:
        aa = rep["adv_rate"].dropna()
        if not aa.empty:
            meta["adv_rate_col"] = float(aa.mode().iloc[0])
    if "label" in rep.columns:
        # 假定 label 列里 adv/honest 之类
        adv_mask = rep["label"].astype(str).str.lower().eq("adv")
        if adv_mask.any():
            meta["adv_frac"] = float(adv_mask.mean())

    if "worker_id" in rep.columns:
        meta["n_workers_data"] = int(rep["worker_id"].nunique())
...
def transform_reports_for_suite(rep_s: pd.DataFrame, mal_workers: set,
    ...
    """
    ★ 修改版：
    - 不再基于 mal_workers 重新造一遍恶意噪声（你的 reports.csv 自己已经 encode 好噪声/攻击结构）。
    - 只根据 rho 做“参与率”裁剪（如果 0<rho<1），否则直接用全部 worker。
    """
    ...
    # 参与率（整窗按 worker 抽样）。rho>=1 或 rho<=0 则不抽样。
    if "worker_id" in df.columns and 0.0 < float(rho) < 1.0:
    ...
    # ★ 不再对恶意 worker 的 value 做额外改动：
    # 我们假定 reports.csv 里的 value 就是“真实观测”（已含 DP 噪声 / 攻击行为）。
    return df
...
def iter_rounds_suite(rep: pd.DataFrame, tru: pd.DataFrame, slots,
    ...
    # ★ 这里不再根据 mal_rate “重采样恶意工人”，
    #   mal_rate 只作为算法内部的先验（Pn.mal_rate）使用。
    all_workers = rep["worker_id"].unique().tolist() if "worker_id" in rep.columns else []
    mal_workers = set()  # transform_reports_for_suite 现在不会用它改值
...
def build_params(args, eps):
    ...
    return _P(
        ...
        A_budget_ratio=float(getattr(args, "A_budget_ratio", 0.25)),
...
def main():
    ...
    # NYC 数据集路径
    ap.add_argument('--reports_csv', default='/Users/wanghao/Desktop/SA-HTD/dataset_use/metr-LA/data/reports.csv')
    ap.add_argument('--truth_csv',   default='/Users/wanghao/Desktop/SA-HTD/dataset_use/metr-LA/data/truth.csv')
    ap.add_argument('--outdir',      default='/Users/wanghao/Desktop/SA-HTD/dataset_use/metr-LA/result')

    ap.add_argument('--n_workers', type=int, default=300)  # ★ 修改：默认 -1 表示自动推断
    ...
    ap.add_argument('--A_budget_ratio', type=float, default=0.25)
